...
def run_commands(device_data):
    ...
    try:
        ...
        data = {}
        commands = ["no monitor capture buffer test",
                    "no monitor capture point ip process-switched point1 both",
                    "no monitor capture point ip cef point1 gig0/1 both",
                    "no monitor capture point ip cef point2 gig0/2 both",
                    "no monitor capture point ip cef point3 gig0/3 both",
                   ]
...
def main(master):
    ...
    with concurrent.futures.ThreadPoolExecutor(15) as executor:
        ...
        for device, device_data in master.items():
            print(f"Deleting capture on {device}..." + "\n")
