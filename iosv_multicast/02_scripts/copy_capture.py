...
scp_user = input("Enter SCP username: ")
scp_pwd = getpass.getpass(prompt = "Enter SCP password: ")
scp_path = input("Enter SCP path: ")
...
def run_commands(device_data):
    ...
    try:
        ...
        data = {}
        command = f"monitor capture buffer test export scp://{scp_user}:{scp_pwd}@{scp_host}/{scp_path}/{device}/{datetime.now}.pcap"
        netconnect.send_command(command)
